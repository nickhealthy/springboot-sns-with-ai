...
## Review Checklist (Total: 16 items)

### Spring API Rules (10 items)
...
3. `ResponseEntity<T>` return type in controllers
4. DTOs as Java records
5. Request DTO: `toEntity()` method / Response DTO: `from(Entity)` static factory (only if needed)
6. No business logic in DTOs (only data conversion methods allowed)
7. Entity: `protected` default constructor, `@GeneratedValue(IDENTITY)`
8. No FK constraints; use `@JoinColumn` only
9. `@Transactional` only when necessary (Dirty Checking, multiple writes)
