...
```
com.nickhealthy.springboot_sns_sample
...
- Use Lombok as the default creator.

## Controller

- Use `@RestController`
- Use `@RequestMapping` at class level for the base path (e.g. `@RequestMapping("/api/users")`)
- Write only the sub-path on each method mapping (e.g. `@GetMapping("/{id}")`)
...
- `protected` default constructor: use `@NoArgsConstructor(access = AccessLevel.PROTECTED)` (not explicit `protected ...() {}`)
